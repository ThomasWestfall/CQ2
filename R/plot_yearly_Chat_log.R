plot_yearly_Chat_log <- function(site.id,model.name,model.short,data){
  ...
  # for(p in 1:length(uniqueyears)){
    ...
    pframe <- pad(pframe,interval = "day")
    plot(pframe, type="l", col="#0072B4", xlim = xlim.date, ylim = ylim.Q, axes = FALSE, xlab="",ylab = "", lwd=2.5, cex.axis = 2,cex.lab = 2, las = 1)
    mtext("log10 Discharge (mm/day)", cex = 2, side = 4, line = 5)
    ...
    if(model.name == "Chat1" || model.name == "Chat1_s" ){
      ...
      legend(x = "top", y = par("usr")[4] + 0.1, legend = c("Obs. C",model.name,"Q"),
             col = c("darkgray","#8856a7","#0072B4"),lty = c(1,1,1),lwd=3, pt.cex = 0.1, pch = 16,cex = 1.2,
      ...
      legend(x = "top", y = par("usr")[4] + 0.1, legend = c("Obs. C","Chat1",model.name,"Q"),
             col = c("darkgray","#8856a7","#fc8d59","#0072B4"),lty = c(1,1,1,1),lwd=3, pt.cex = 0.1, pch = 16,cex = 1.2,
      ...
      par(new=TRUE)
      pframe <- data.frame(datetime,log10(data[[paste("Q",model.short,sep="")]]))
      pframe <- pad(pframe,interval = "day")
      ...
      legend(x = "top", y = par("usr")[4] + 0.1, legend = c("Obs. C","Chat1",model.name,"Q",paste("Q",model.short,sep="")),
             col = c("darkgray","#8856a7","#fc8d59","#0072B4","#C3DBFD"),lty = c(1,1,1,1,1),lwd=3, pt.cex = 0.1, pch = 16,cex = 1.2,
