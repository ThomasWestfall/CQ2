Chat9 <- function(params, flow, flow.s, flow.date) { #quick-slow CQ variant,
  #source load varies as slow and quick flow vary with hystersis term
  ...
  if(NCOL(params)>1){
    ...
    # forward and backward difference at record beginning and end
    flow.d[1,] <- (flow[2,] -flow[1,])/1
    flow.d[nrow(flow),] <- (flow[nrow(flow),] - flow[(nrow(flow)-1),])/1

    # forward and backward difference at beginning and end of each continous period
