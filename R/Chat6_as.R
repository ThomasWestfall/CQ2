Chat6_as <- function(params, flow, flow.s, flow.date, dec.time) { #C ~ Qs, Qs/Q
  ...
  if(NCOL(params)>1){
    ...
    # c_disp <- params[7,]^5
    #
    # c <- sweep(sweep(sin(sweep(2*pi*dec.time,MARGIN = 2,c_phase,'+')),MARGIN = 2, c_amp, '*',),MARGIN = 2, c_disp,"+")
    ...
    Pred <- as*sweep(flow.s, MARGIN=2,bs, `^`) +  sweep(flow.q.d,Margin=2,c,'*')
    ...
    # c_disp <- params[7]^5
    #
    # c <- c_amp*sin(c_phase+2*pi*dec.time)+c_disp
